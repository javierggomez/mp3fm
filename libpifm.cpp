...
void setupDMA( float centerFreq ){
  ...
  signal (SIGQUIT, handSig);
   ...
   while (instrCnt<1024) {
     ...
     for (unsigned int i=0; i<4096/sizeof(struct CB); i++) {
...
void playStream(TSQueue<short>* stream);

void run_fm(TSQueue<short>* stream, double freq) {
  setup_fm();
  setupDMA(freq);
  playStream(stream);
}
